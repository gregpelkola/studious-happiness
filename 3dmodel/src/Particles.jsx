...
export default function Particles({ particlesCount = 50 }) {
    ...
    const particlesMaterial = useMemo(() => {
        return new THREE.ShaderMaterial({
            ...
            vertexShader: THREE.ShaderChunk.points_vert.vertexShader,
            fragmentShader: `
                uniform vec3 color;
                void main() {
                    vec2 xy = gl_PointCoord.xy - vec2(0.5);
                    float ll - length(xy);
                    gl_FragColor = vec4(color, step(11, 0.5) * 0.25);
                }
            `
        })
    }, [])
    ...
    useFrame((state, delta) => {
        if (particlesRef.current) {
            const positions = particlesRef.current.geometry.attributes.postion.array
